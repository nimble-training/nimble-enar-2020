...
*nimbleFunctions* are at the heart of NIMBLE. They are the way that algorithms are implemented. They can also be used for user-defined functions and samplers for use in BUGS code, and they can be used to compile parts of R, without reference to a model. But their main purpose is providing a way for developers to implement algorithms.
...
Chapter 10 of the NIMBLE User Manual describes the syntax for *run* code in detail, including lots of neat functionality such as using nested nimbleFunctions and having multiple run-time functions (i.e., methods) as part of a nimbleFunction.
...
Then we'll use it to simulate from the GLM model defined above (e.g., if we wanted to set up a simulation study).
...
Note that a `modelValues` object cannot be passed as an argument or a return value in run code.  Instead it must be created or shared among nimbleFunctions via setup code and accessed from R as a member object.
...
# A basic demonstration of compiling R code

Consider generating from a time series model. It can't easily be done using vectorized R code because of the serial dependence.

We can use NIMBLE to compile the code and run it much more quickly.

```{r, compile-r, fig.cap="", fig.height=5, fig.width=5}
set.seed(1)
n <- 1e6
path <- rep(0, n)
rho1 <- .8
...
Not bad: going to C++ gives us a speedup of approximately 40-fold. And all we had to do was annotate our code with some type information.
